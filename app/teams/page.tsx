      ...
      <div className="flex flex-col items-start gap-4 md:flex-row md:justify-between md:items-center mb-8">
        <div>
          ...
          <p className="text-muted-foreground">
            View all teams participating in the JK2 No More Outcasts CTF Tournament.
          </p>
        </div>
        <DataStatus />
      </div>
      ...
      <div className="grid gap-6 lg:grid-cols-4">
        ...
        <div className="lg:col-span-3 grid gap-6 sm:grid-cols-2 lg:grid-cols-3">
          {sortedTeams.map((team, index) => {
            ...
            const teamCaptain = teamPlayers.find((player) => player.role === "Captain")

            return (
              <Card
                key={team.id}
                className={`overflow-hidden transition-all hover:shadow-md ${getTeamColorClass(team.name)}`}
              >
                <CardHeader className="pb-3 relative">
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      <CardTitle className="flex items-center gap-2">{team.name}</CardTitle>
                ...
                <CardContent className="pb-3">
                  <div className="flex items-center justify-between mb-4 bg-background/80 p-2 rounded-md">
                    <div className="flex items-center gap-2">
                      ...
                      <span className="text-sm">{team.stats.matchesWon} Wins</span>
                    </div>
                    <div className="flex items-center gap-2">
                      <Flag className="h-4 w-4 text-muted-foreground" />
                      <span className="text-sm">{team.stats.captures} Captures</span>
                    </div>
                    <div className="flex items-center gap-2">
                      <Users className="h-4 w-4 text-muted-foreground" />
                      <span className="text-sm">{teamPlayers.length} Players</span>
                    </div>
                  </div>
                  ...
                  <div className="space-y-3">
                    ...
                    <div className="space-y-2">
                      {teamCaptain && (
                        <div className="flex items-center gap-2 p-2 bg-background/80 rounded-md">
                          ...
                          <div className="flex flex-col">
                            <span className="font-medium">{teamCaptain.name}</span>
                      ...
                      <div className="flex flex-wrap gap-2">
                        ...
                        {teamPlayers.filter((player) => player.role !== "Captain").length > 4 && (
                          <Badge variant="outline">
                            +{teamPlayers.filter((player) => player.role !== "Captain").length - 4} more
                          </Badge>
                        )}
                      </div>
                    </div>
